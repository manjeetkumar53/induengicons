...
import { Minus, Receipt, Edit3, FileImage } from 'lucide-react'
...
export default function ExpenseGrid({ 
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/admin/accounting/transactions?type=expense')
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-r from-indigo-500 to-indigo-600 rounded-lg p-6 text-white">
          <div className="flex items-center">
            <div className="flex-1">
              <p className="text-indigo-100 text-sm">Top Category</p>
              <p className="text-lg font-bold">
                {topCategory ? topCategory[0] : 'N/A'}
              </p>
              <p className="text-indigo-200 text-sm">
                {topCategory ? `₹${topCategory[1].toLocaleString('en-IN')}` : ''}
              </p>
            </div>
      ...
      <DataGrid
        ...
        subtitle={`Manage all expense transactions and receipts`}
        columns={columns}
        data={transactions}
        loading={loading}
