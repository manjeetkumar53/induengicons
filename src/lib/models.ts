...
CompanySchema.index({ 'registration.gst': 1 }, { sparse: true })
CompanySchema.index({ 'registration.pan': 1 }, { sparse: true })
...
TransactionCategorySchema.index({ 'accounting.code': 1 }, { unique: true })
...
const SimpleTransactionSchema = new Schema({
  ...
  paymentMethod: { 
    type: String, 
    enum: ['cash', 'bank', 'card', 'cheque', 'upi', 'neft', 'rtgs', 'other'], 
    required: true 
  },
...
// Transaction indexes
SimpleTransactionSchema.index({ transactionNumber: 1 }, { unique: true })
...
// Allocation indexes
SimpleAllocationSchema.index({ allocationNumber: 1 }, { unique: true })
